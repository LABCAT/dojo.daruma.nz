...
export default function DojoIndex() {
  console.log('🟡 [DojoIndex] RENDER — TOP OF FUNCTION')
  
  // === DEBUG: wrap every hook to find the thrower ===
  let router: ReturnType<typeof useRouter>
  try {
    router = useRouter()
    console.log('🟡 [DojoIndex] useRouter() OK')
  } catch (e) {
    console.log('🟡🔥🔥🔥 [DojoIndex] useRouter() THREW:', e)
    throw e
  }
  ...
  useEffect(() => {
    console.log('🟡 [DojoIndex] MOUNTED')
    return () => {
      console.log('🟡🟡🟡 [DojoIndex] UNMOUNTED')
    }
  }, [])
  ...
  try {
    useFocusEffect(
      useCallback(() => {
        console.log('🟡 [DojoIndex] useFocusEffect FIRED — deferring MMKV read via InteractionManager')
        const task = InteractionManager.runAfterInteractions(() => {
          console.log('🟡 [DojoIndex] InteractionManager callback — NOW reading MMKV')
          try {
            const preset = getActiveDifficulty()
            console.log('🟡 [DojoIndex] preset:', preset)
            setActivePreset(preset)
            const newStatuses = getAllRankStatuses(preset)
            console.log('🟡 [DojoIndex] statuses:', JSON.stringify(newStatuses))
            setStatuses(newStatuses)
            console.log('🟡 [DojoIndex] setState calls completed')
          } catch (e) {
            console.log('🟡🔥 [DojoIndex] InteractionManager callback THREW:', e)
          }
        })
        return () => {
          console.log('🟡 [DojoIndex] useFocusEffect CLEANUP — cancelling InteractionManager task')
          task.cancel()
        }
      }, [])
    )
    console.log('🟡 [DojoIndex] useFocusEffect() hook call OK')
  } catch (e) {
    console.log('🟡🔥🔥🔥 [DojoIndex] useFocusEffect() THREW:', e)
    throw e
  }

  let theme: ReturnType<typeof getDifficultyTheme>
  try {
    theme = getDifficultyTheme(activePreset)
    console.log('🟡 [DojoIndex] RENDER COMPLETE — about to return JSX')
  } catch (e) {
    console.log('🟡🔥 [DojoIndex] getDifficultyTheme THREW:', e)
    throw e
  }
