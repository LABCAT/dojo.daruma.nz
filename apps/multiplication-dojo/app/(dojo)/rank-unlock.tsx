...
import { advanceRank, RANKS, getDifficultyTheme } from '@daruma/ui'
...
export default function RankUnlock() {
  ...
  // 2. Advance the rank in storage
  useEffect(() => {
    console.log('🟢 [RankUnlock] advanceRank useEffect — preset:', preset, 'completedId:', completedId)
    if (params.preset && params.completedRankId) {
      try {
        const result = advanceRank(preset, completedId)
        console.log('🟢 [RankUnlock] advanceRank returned:', result)
      } catch (e) {
        console.log('🟢🔥 [RankUnlock] advanceRank THREW:', e)
      }
    }
  }, [preset, completedId, params.preset, params.completedRankId])

  // Track mount/unmount
  useEffect(() => {
    console.log('🟢 [RankUnlock] MOUNTED')
    return () => {
      console.log('🟢🟢🟢 [RankUnlock] UNMOUNTED')
    }
  }, [])
  ...
  useEffect(() => {
    ...
    // Trigger haptics right when the spring hits
    setTimeout(() => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#0A0A0A', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <Animated.View style={{ position: 'absolute', bottom: 64, opacity: btnOpacity, width: '100%', alignItems: 'center' }}>
        <Pressable
          onPress={() => {
            console.log('🟢 [RankUnlock] Continue PRESSED — about to navigate')
            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)
            try {
              console.log('🟢 [RankUnlock] calling router.dismissAll() — stack should be [index, challenge, rank-unlock]')
              router.dismissAll()
              console.log('🟢 [RankUnlock] dismissAll returned successfully')
            } catch (e) {
              console.log('🟢🔥 [RankUnlock] dismissAll THREW:', e)
            }
          }}
        >
          {({ pressed }) => (
